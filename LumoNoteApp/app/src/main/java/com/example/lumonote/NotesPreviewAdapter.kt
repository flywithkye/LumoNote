...
class NotesPreviewAdapter(private var notesList: List<NoteItem>, context: Context)
    : RecyclerView.Adapter<NotesPreviewAdapter.NotePreviewViewHolder>()
{
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NotePreviewViewHolder {
        val notePreviewView = LayoutInflater.from(parent.context).inflate(R.layout.note_preview,
            parent, false)
    ...
    fun refreshData(newNotes: List<NoteItem>) {
        notesList = newNotes
